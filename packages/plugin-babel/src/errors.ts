...
// Re-export error types from internal/state for public API
export type { PluginError } from "./internal/state";

/**
 * Type alias for plugin operations that can fail
 */
export type PluginResult<T> = Result<T, import("./internal/state").PluginError>;

/**
 * Shorthand for creating an Err result with a PluginError
 */
export const pluginErr = <T>(error: import("./internal/state").PluginError): PluginResult<T> => {
  return err(error);
};
 ...
 */
export const isPluginError = (value: unknown): value is import("./internal/state").PluginError => {
  return (
    typeof value === "object" &&
    value !== null &&
    "type" in value &&
 ...
 */
export const formatPluginError = (error: import("./internal/state").PluginError): string => {
