...
describe("TypeScript Compiler Plugin Integration", () => {
  const fixturesDir = join(import.meta.dir, "../fixture-catalog/fixtures/core/valid");
  const sourceFile = join(fixturesDir, "sample.ts");

  test(
    "should accept new configuration options",
    () => {
  ...
  describe("ESM Output", () => {
    test(
      "should use import and plain gqlRuntime for ESM output",
      () => {
        const compilerOptions: ts.CompilerOptions = {
          target: ts.ScriptTarget.ES2020,
          module: ts.ModuleKind.ESNext,
  ...
  describe("CommonJS Output", () => {
    test(
      "should use require and __soda_gql_runtime accessor for CJS output",
      () => {
        const compilerOptions: ts.CompilerOptions = {
          target: ts.ScriptTarget.ES2020,
    ...
    test(
      "should handle Node16/NodeNext module resolution with CJS files",
      () => {
        const compilerOptions: ts.CompilerOptions = {
          target: ts.ScriptTarget.ES2020,
