 ...
 * Base post fragment
 */
export const postFragment = gql.default(({ fragment }) => fragment.Post({}, ({ f }) => [f.id(), f.title()]));
...
export const userWithPostsFragment = gql.default(({ fragment }, { $var }) =>
  fragment.User(
    ...
    ({ f, $ }) => [f.id(), f.name(), f.posts({ categoryId: $.categoryId })(() => [postFragment.embed()])],
  ),
);
...
export const getUserWithPostsQuery = gql.default(({ query }, { $var }) =>
  query.operation(
    ...
    ({ f, $ }) => [f.user({ id: $.userId })(() => [userWithPostsFragment.embed({ categoryId: $.categoryId })])],
  ),
);
