...
export type JsonCacheFactory = {
	createStore<K extends string, V>(
		options: JsonCacheStoreOptions<K, V>,
	): JsonCacheStore<K, V>;
	clearAll(): void;
};
...
const toNamespacePath = (root: string, segments: CacheNamespace): string =>
	join(root, ...segments.map(sanitizeSegment));

const toEntryFilename = (key: string): string =>
	`${Bun.hash(key).toString(16)}${JSON_EXT}`;

export const createJsonCache = ({
	rootDir,
	prefix = [],
	...
	return {
		createStore: <K extends string, V>({
			...
			const resolveEntryPath = (key: string) =>
				join(storeRoot, toEntryFilename(key));
