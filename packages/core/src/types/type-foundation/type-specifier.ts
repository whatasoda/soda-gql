...
// Record types use deferred string format for performance
// Note: Also accepts DeferredOutputFieldWithArgs at runtime, but typed as string for backward compat
export type OutputTypeSpecifiers = { readonly [key: string]: DeferredOutputSpecifier };
 ...
 * - Inline arguments format (legacy): "argName:k|Type|Mod,..."
 * - New format uses DeferredOutputFieldWithArgs object instead
 */
export type DeferredOutputSpecifier = `${"s" | "e" | "o" | "u"}|${string}|${TypeModifier}${string}`;

/**
 * Deferred output specifier without inline arguments.
 * Used as the `spec` property in DeferredOutputFieldWithArgs.
 */
export type DeferredOutputSpecifierSimple = `${"s" | "e" | "o" | "u"}|${string}|${TypeModifier}`;
...
export type DeferredOutputFieldWithArgs = {
  readonly spec: DeferredOutputSpecifierSimple;
  readonly arguments: InputTypeSpecifiers;
};
