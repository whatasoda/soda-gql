...
describe('QuerySlice type', () => {
  ...
  it('should support selection builder', () => {
    ...
    expect(mockBuilder.argument('limit', 10)).toEqual({ name: 'limit', value: 10 })
  })
  ...
  it('should support transform function', () => {
    type RawData = { user: { id: string; first_name: string; last_name: string } }
  ...
  it('should support complex arguments', () => {
    ...
    const slice: QuerySlice<SearchData, SearchArgs> = {
      _data: { results: [], total: 0 },
      _args: {
        query: '',
        filters: { status: [], dateRange: { from: new Date(), to: new Date() } },
