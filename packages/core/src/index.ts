...
const createArgsNormalizer =
  <TSchema extends AnyGraphqlSchema, TTypeName extends keyof TSchema["object"] & string>() =>
  <TFieldName extends keyof TSchema["object"][TTypeName]["fields"] & string>(
    value: VariableReferencesByFieldName<TSchema, TTypeName, TFieldName> | undefined,
  ): VariableReferencesByFieldName<TSchema, TTypeName, TFieldName> =>
  ...
  fieldEntries.forEach((fieldName) => {
    ...
    if (field.type.kind === "union") {
      const factory = (
        ...
        const unionBuilder =
          typeof args === "object" && args !== null && !("parent" in (args as object))
            ? (args as {
                [TMember in keyof TSchema["union"][typeof field.type.name]["types"] & string]: (tools: {
                  _: FieldReferenceFactories<TSchema, TMember & keyof TSchema["object"]>;
                  f: FieldReferenceFactories<TSchema, TMember & keyof TSchema["object"]>;
        ...
        Object.keys(schema.union[field.type.name].types).forEach((memberName) => {
          const nestedFactories = createFieldFactories(schema, memberName as keyof TSchema["object"] & string);
    ...
    const factory = (args?: VariableReferencesByFieldName<TSchema, TTypeName, typeof fieldName>) => ({
      [fieldName]: {
        parent: typeName,
        field: fieldName,
        type: field.type,
...
const createVariableAssignments = <TSchema extends AnyGraphqlSchema, TVariables extends { [key: string]: InputDefinition }>(
...
const isSliceSelectionSingle = <TAdapter extends GraphqlAdapter>(
  value: AnySliceResultSelections<TAdapter>,
): value is SliceResultSelection<TAdapter, string, unknown, unknown> =>
  typeof value === "object" && value !== null && "transform" in value && "path" in value;

const evaluateSelections = <TSchema extends AnyGraphqlSchema, TAdapter extends GraphqlAdapter, TFields extends AnyFields>(
...
const createSliceSelector =
  <TSchema extends AnyGraphqlSchema, TAdapter extends GraphqlAdapter, TFields extends AnyFields>(
    _schema: TSchema,
    _fields: TFields,
  ) =>
...
const createOperationSliceFactory =
  <TSchema extends AnyGraphqlSchema, TAdapter extends GraphqlAdapter>(schema: TSchema, adapter: TAdapter) =>
...
export const createGql = <TSchema extends AnyGraphqlSchema, TAdapter extends GraphqlAdapter>({
  ...
  const model: ModelFn<TSchema> = (target, builder, transform) => {
    ...
    const fieldFactories = createFieldFactories(schema, typename as keyof TSchema["object"] & string);
    ...
    return {
      ...
      fragment: (assignments) =>
        builder({
          ...
          f: fieldFactories,
          fields: fieldFactories,
          $: createVariableAssignments<TSchema, typeof variablesDefinition>(variablesDefinition, assignments),
        }),
      transform,
    };
  };
