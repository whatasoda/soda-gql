...
describe("Plugin-Babel Behavioral Tests", () => {
	describe("Operations", () => {
		it("transforms operations correctly", async () => {
			...
			const transformed = await runBabelTransform(
				fixture.sourceCode,
				fixture.sourcePath,
				fixture.artifact,
				{ skipTypeCheck: true },
	...
	describe("Import Hygiene", () => {
		it("adds runtime import when needed", async () => {
			const fixture = await loadPluginBabelFixture(
				"imports/add-runtime-import",
			);
		...
		it("merges with existing runtime import", async () => {
			...
			expect(transformed).toContain('from "@soda-gql/runtime"');
			const importCount = (
				transformed.match(/from "@soda-gql\/runtime"/g) || []
			).length;
