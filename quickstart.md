...
Edit `./src/graphql-system/inject.ts` to describe each custom scalar and to implement the `GraphqlAdapter`. The template uses the new `defineScalar()` helper so each scalar stays strongly typed.
