...
const internalPostFragment = gql.default(({ fragment }) =>
  fragment("InternalPostFragment", "Task")`{ id title completed }`(),
);
...
export const userWithPostsFragment = gql.default(({ fragment }) =>
  fragment("UserWithPostsFragment", "Employee")`{ id name tasks { id title completed } }`(),
);
...
export const nestedQueries = {
  users: {
    list: gql.default(({ query }) =>
      query("NestedUserList")`($limit: Int) { employees(limit: $limit) { id name email } }`(),
    ),
