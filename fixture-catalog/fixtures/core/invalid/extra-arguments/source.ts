...
export const extraArguments = gql.default(({ fragment }) => fragment.User({ fields: ({ f }) => ({ ...f.id() }) }), extraArg);

// Multiple extra arguments
// @ts-expect-error - intentionally invalid for testing (extra args not allowed by type)
export const multipleExtras = gql.default(({ fragment }) => fragment.User({ fields: ({ f }) => ({ ...f.name() }) }), "option1", "option2");
